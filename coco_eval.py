...
def evaluate_detection(gt_boxes_list, dt_boxes_list, classes=("car", "pedestrian"), height=240, width=304,
    ...
    # create a file to store the results in (timestamp, x, y, w, h, class, confidence) in npx format
    # ----------------------------------------------
        

    detection = []
    for items in flattened_dt:
        detection.extend(items)
    detection = np.array(detection)
        

        # ----------------------------------------------

    # print first and the last gt
    # print('first gt:', flattened_gt[0])
    # print('last gt:', flattened_gt[-1])

    # # print shape
    # print('flattened_gt shape:', len(flattened_gt))
    # print('flattened_dt shape:', len(flattened_dt))
    # print('detection shape:', detection.shape)

    # # save with pickle
    # with open('flattened_gt_test.pkl', 'wb') as f:
    #     pickle.dump(flattened_gt, f)
    # with open('flattened_dt_test.pkl', 'wb') as f:
    #     pickle.dump(flattened_dt, f)

    # # save print output as text file
    # print('----------------origninal dataset----------')
    # print('gt:', flattened_gt[:10])
    # print('dt:', flattened_dt[:10])

        # ----------------------------------------------


    file_name = f'detections.npy'
    np.save(file_name, detection)
    print(f'-------------------------Saved detections to {file_name}--------------------------------------------')
...
def _coco_eval(gts, detections, height, width, labelmap=("car", "pedestrian"), return_aps: bool = True):
    ...
    out_keys = ('AP', 'AP_50', 'AP_75', 'AP_S', 'AP_M', 'AP_L', 'AP_car', 'AP_pedestrian')
    ...
    if return_aps:
        with open(os.devnull, 'w') as f, contextlib.redirect_stdout(f):
            ...
            coco_eval.summarize()
        for idx, key in enumerate(out_keys[:6]):
            out_dict[key] = coco_eval.stats[idx]
        
        #  Compute AP for each class
        for class_id, class_name in enumerate(labelmap):
            coco_eval = COCOeval(coco_gt, coco_pred, 'bbox')
            coco_eval.params.imgIds = np.arange(1, len(gts) + 1, dtype=int)
            coco_eval.params.catIds = [class_id + 1]
            coco_eval.evaluate()
            coco_eval.accumulate()
            coco_eval.summarize()
            out_dict[f'AP_{class_name}'] = coco_eval.stats[0]  # Index 0 for overall AP
